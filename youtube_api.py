...
class Channel:
    def __init__(self, channel_name, channel_alias="", channel_id=None):
    ...
    def _getVideosBetween(self, since_date, to_date):
        ...
        while not foundAll:
            try:
                req = requestChannelVideosInfo.format(
                    self.channelId, to_date, since_date, nextPToken)
    ...
    def getAllVideos(self):
        ...
        firstDate = datetime(year=2005, month=4, day=22)
        todayDate = datetime.now()

        return self._getVideosBetween(firstDate, todayDate)

    def getAliasOrName(self):
        if len(self.channelAlias) == 0:
            return self.channelName
        else:
            return self.channelAlias
...
class Video:
    ...
    def getData(self, parse_duration=True, parse_date=True):
        try:
            ...
            results["url"] = "https://www.youtube.com/watch?v={}".format(
                self.videoId)
